				...
				<div
					...
					<div className={'flex w-full flex-row items-center justify-between'}>
						...
						<p className={`${option.icon ? 'pl-2' : 'pl-0'} text-xs font-normal text-neutral-600`}>
							{`Base APY ${formatPercent(option?.value?.APY || 0, 2, 2, 500)}`}
						</p>
					</div>
				</div>
			)}
				...
				<>
					<Combobox.Button
						...
						<div className={'relative flex flex-row items-center'}>
							<div
								key={selected?.label}
								className={'size-6 flex-none rounded-full border border-neutral-100'}>
								{selected?.icon ? (
									cloneElement(selected.icon)
								) : (
									<div className={'size-6 flex-none rounded-full bg-neutral-900'} />
								)}
							</div>
							<p
								className={
